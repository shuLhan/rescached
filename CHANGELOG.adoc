...
// SPDX-FileCopyrightText: 2019 M. Shulhan <ms@kilabit.info>S
...
[#rescached_v4_6_0]
==  rescached v4.6.0 (2026-05-15)

This minor release is due to the Go module and the repository of this
project has been migrated to new domain.

The Go module path now is "kilabit.info/rescached" and the repository is
https://git.kilabit.info/rescached .

We also changes the license to GPL-3.0-only.
The "or-later" suffix on previous license, GPL-3.0-or-later, is too broad.
Lets limit to what we have now, which is GPL-3.0-only.

Other changes are noted below.

**🌼 Refactoring DoT and DoH to use listen address instead port**
